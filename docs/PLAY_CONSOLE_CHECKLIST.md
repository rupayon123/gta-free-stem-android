...
- [ ] Exact v1.0.1 signing identity and increasing version code confirmed before any upload.
...
- [ ] `./scripts/verify-release-config.sh` passes for the intended version.
- [ ] `REQUIRE_SIGNING=1 ./scripts/verify-release-config.sh` passes locally without exposing secrets.
...
- [ ] Exact v1.0.1 release candidate built, signed, inspected, and identified without exposing secrets.
...
- [ ] Exact source commit, AAB SHA-256, version, and upload certificate fingerprint independently cross-checked.
